...
# Ce programme detecte les orthologues a partir d'un arbre PhyML au format Newick donne en argument. 
...
# mort! (???)
# Les arbres sont representés par des tableaux de hashage: 
# Un pour les enfants, et un pour les parents.


# Reste a reprendre la distance PAM pour pouvoir decider d'une paire d'Orthologues 
# lorsque on a une proteine face a 1 groupe de paralogues
# et lorsqu'on a un groupe de paralogues face a un groupe de paralogues.
...
sub newDPam
{
    ...
    unless(defined $fichier)
    {
		die "You have to give the file name in Arbre->new(file)\n";
	}
...
sub analyse
{
    ...
    close(IN);
	#suppression des valeurs des distances dans l'arbre PhyML
    $s =~ s/\;//g;
    $s =~ s/:.*?([,\)])/$1/g; 
    #Ex: (((8_490,((((2_3593,4_1109),5_4032),1_3802),7_1285)),9_2887),(6_3486,(3_4661,0_1028)))
    
    # On transforme la chaine obtenue en un tableau de caractere
    my @t = split(//,$s);

    # Initialisation des variables de la boucle de parcourt de la chaine
...
# parsing des fichiers groupe_<numero du groupe> dans STEP2_<tag> posterieurement au run de mcl (STEP3)
# recupere la valeur de distance pour les 2 proteines de chaque edge dans un hash (ratio de scores de blast)
sub analyse_dPam
{
    my ($this,$fichier) = @_;
    if (-e $fichier)
...
# Analyse l'arbre en question pour savoir si c'est un arbre contenant:
#   - 1 seule proteine par espece:    retourne 0
#   - 1 seule proteine par espece ou des inparalogs :       retourne 1
#   - des out-paralogs (avec ou sans in in-paralogs a d'autres positions): retourne 2
sub simplicite
{
    my ($this) = @_;
...
# Recherche des out-paralogs dans l'arbre
# La fonction parcourt l'arbre depuis la racine et identifie les proteines de même espece
# Renvoie vrai si les enfants du noeud ne sont pas des feuilles
sub outpara
{
    my ($this,$noeud) = @_;
...
# Retourne vrai (1) si le noeud en argument est une feuille sinon faux
sub feuille
{
    my ($this,$noeud) = @_;
    if(!(defined $noeud) || (!exists( $this->{enfants}{$noeud}) && !(exists $this->{parent}{$noeud})))
...
# Renvoie vrai si le noeud en argument provient d'une duplication
...
sub duplication
{
	...
	# Extraction des proteines enfants du noeud en argument
    ...
    my $intersect = 0;
    # generation de tous les couples de proteines a partir de la liste tabEnfants
    for(my $i=0;$i<=$#tabEnfants && !$intersect;$i++)
    {
		for(my $j=$i+1;$j<=$#tabEnfants && !$intersect;$j++)
		{
			# la fonction Intersect compare l'origine des enfants de 2 proteines (true = même origine)
...
sub parcourOrtho
{
    ...
    {
		#print "$noeud : case is duplication\n";
    ...
    {
		#print "$noeud : case is no duplication\n";
