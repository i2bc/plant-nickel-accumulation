...
my $blast = $ARGV[0] ; # INPUT : nom de la premiere espece
my @suffixlist = (".blast");
my ($blastname, $blast_dir, $suffix)  = fileparse ($blast, @suffixlist);
print $blastname,'**' ,$blast_dir,'**' ,$suffix, "\n";
my @blastname = split('_vs_', $blastname);
my $spA = $blastname[0];
my $spB = $blastname[1];
my $result_dir = "Phylogeny/STEP1/";
my $evalue_cutoff = $ARGV[1]; # INPUT : seuil sur la evalue pour former familles d'homologues
my $alignment_cutoff = $ARGV[2]; # INPUT : seuil sur le pourcentage d'alignement pour former familles d'homologues
